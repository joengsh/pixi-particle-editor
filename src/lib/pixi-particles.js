...
var PathParticle = /** @class */ (function (_super) {
    ...
    PathParticle.prototype.update = function (delta) {
        ...
        if (lerp >= 0 && this.path) {
            ...
            // customised update: use svg path getPointAtLength() to compute the position instead
            if (this.movement> this.pathLength) {
                this.age = this.maxLife;
            }
            const dist = Math.min(this.movement, this.pathLength)
            const pt = this.path(this.movement);
            helperPoint$1.x = pt.x;
            helperPoint$1.y = pt.y;
    ...
    PathParticle.parseData = function (extraData) {
        ...
        if (extraData && extraData.path && typeof extraData.path === 'function') {
            try {
                ...
                output.pathLength = extraData.pathLength;
            }
            catch (e) {
                if (ParticleUtils.verbose) {
